...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $qry = "SELECT * FROM donations WHERE receipt_date BETWEEN ? AND ?";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $donations[] = $row;
            $totalAmount += is_numeric($row['amount']) ? $row['amount'] : 0; // Calculate total amount
        }
    } else {
        $error = "No donations found in the given date range.";
    }
}
?>
                            ...
                            <tbody>
                                <?php foreach ($donations as $index => $donation): ?>
                                    <tr>
                                        ...
                                        <td><?php echo $donation['transaction_id']; ?></td>
                                        <td><?php echo $donation['receipt_date']; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
